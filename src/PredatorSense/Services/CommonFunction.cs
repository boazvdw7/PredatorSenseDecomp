...
namespace PredatorSense
{
	...
	public class CommonFunction
	{
		...
		public static bool UpdateImage(Image sender, string name)
		{
			try
			{
				string normalizedName = (name ?? string.Empty).Replace('/', '\\').TrimStart('\\');
				string imagePath = (Startup._Image_path ?? string.Empty) + normalizedName;
				if (File.Exists(imagePath))
				{
					sender.Source = new BitmapImage(new Uri(imagePath, UriKind.Absolute));
					return true;
				}
				string scale = "100";
				try
				{
					string text = (Startup._Image_path ?? string.Empty).TrimEnd('\\', '/');
				...
				string normalizedForPack = normalizedName.Replace('\\', '/');
				Uri uri = new Uri("pack://application:,,,/images/" + scale + "/" + normalizedForPack, UriKind.Absolute);
			...
			catch (Exception)
			{
				try
				{
					string normalizedName2 = (name ?? string.Empty).Replace('\\', '/').TrimStart('/');
					string scale2 = "100";
					try
					{
						string text2 = (Startup._Image_path ?? string.Empty).TrimEnd('\\', '/');
					...
					Uri uri2 = new Uri("pack://application:,,,/Images/" + scale2 + "/" + normalizedName2, UriKind.Absolute);
					sender.Source = new BitmapImage(uri2);
					return true;
				}
				catch (Exception)
				{
